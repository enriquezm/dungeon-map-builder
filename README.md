...
🔗 **Live Demo:** [https://dungeon-map-builder.onrender.com/](https://dungeon-map-builder.onrender.com/)

![Dungeon Map Builder Screenshot](screenshot.png)

## ✨ Features

- 🗺️ **Grid-based canvas** — Place tiles on a 30×30 grid (36px per cell)
- 🚪 **Room tiles** — 5 room variations with different door configurations (1-door, 2-door, 3-door, 4-door)
- 🛤️ **Hallway tiles** — 4 hallway variations (straight, L-corner, T-intersection, 4-way crossroad)
...
- ↩️ **Undo** — Ctrl/Cmd+Z or click the Undo button to revert actions
...
| Action | Input |
|--------|-------|
| 🖱️ Place tile | Left-click on grid cell |
| 🗑️ Remove tile | Right-click on grid cell |
...
```typescript
{
  ...
  shape: { outline: myTileSvg, fill: "" },
}
```

Door values: `"top"`, `"right"`, `"bottom"`, `"left"`

Don't forget to import the SVG at the top of the file:
```typescript
import myTileSvg from "../assets/tiles/rooms/my-tile.svg";
```
